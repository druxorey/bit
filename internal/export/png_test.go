...
func TestGeneratePNG_ShadeCharacterLightShade(t *testing.T) {
	// Light shade (░) should have reduced brightness (not alpha)
	...
	r, g, b, a := img.At(CellSize/2, CellSize/2).RGBA()
	r8, g8, b8, a8 := uint8(r>>8), uint8(g>>8), uint8(b>>8), uint8(a>>8)
	// Light shade should have ~25% brightness of white = ~64, with full alpha
	if a8 != 255 {
		t.Errorf("expected light shade to be fully opaque (A=255), got A=%d", a8)
	}
	if r8 < 32 || r8 > 96 {
		t.Errorf("expected light shade brightness ~64, got %d", r8)
	}
	if r8 != g8 || g8 != b8 {
		t.Errorf("expected grayscale, got RGB(%d,%d,%d)", r8, g8, b8)
	}
}
